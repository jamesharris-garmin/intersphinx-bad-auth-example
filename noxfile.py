...
nox.options.sessions = ["docs(test_case='good_password_bad_path')"]
...
def install_uv_project(
    ...
    if use_pip_interface:
        ...
        uv_cmd.extend(
            ["--directory", str(req_dir.resolve()), "--output-file", str(req_file.resolve())],
        )
...
@nox.session
@nox.parametrize("test_case", ["bad_password", "good_password_bad_path", "happy_path"])
def docs(session: nox.Session, test_case):
    "Build sphinx documentation with case"

    install_uv_project(session, use_pip_interface=True)

    output_dir = SPHINX_OUTPUT_DIR / test_case
    ...
    try:
        session.run(
            ...
            env={
                "INTERSPHINX_EXAMPLE_TYPE": test_case
            }
        )
    finally:
...
@nox.parametrize("test_case", ["bad_password", "good_password_bad_path", "happy_path"])
def doc_server(session: nox.Session, test_case):
